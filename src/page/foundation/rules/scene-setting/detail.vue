<template>
  <fox-dialog :title="dialogTitle"
      ...
      <fox-main class="left-main">
        <fox-form ref="temp"
          ...
          <section class="rule-section">
            <fox-group class="left-input"
                       column="2"
                       label-width="80px">
              <fox-select-item v-model="input1"
                               v-verify="'require'"
                               :source="sceneStatus"
                               placeholder="请选择">场景码</fox-select-item>
              <fox-select-item v-model="input2"
                               v-verify="'require'"
                               :source="serveStatus"
                               placeholder="请选择">服务码</fox-select-item>
            </fox-group>
            <fox-group class="left-input"
                       column="1"
                       label-width="80px">
              <fox-text-item v-model="input3"
                             placeholder="请选择">描述</fox-text-item>
            </fox-group>
          </section>
          <div v-for="(item,index) in temp.ruleList"
               :key="index">
            ...
            <section class="rule-section">
              <fox-group class="left-input"
                ...
                <fox-text-item v-model="item.ruleName"
                               placeholder="请输入规则名称">规则名称</fox-text-item>
                <fox-select-item v-model="item.isOpen"
                                 :source="m_status"
                                 placeholder="">是否启用</fox-select-item>
              </fox-group>
            </section>
            <section class="rule-section">
              <fox-group class="left-input"
                ...
                <fox-item label="判断条件">
                  <div class="drag-block">
                    <draggable class="condition-area"
                               :list="item.conditionList"
                               v-bind="{group:{name:(item.ruleType==='0'||item.isCondition==0||operation=='detail')?'other':sign },index: index, filter:'.undraggable,', sort:false}"
                               @add="add"
                               @start="start"
                               @end="end"
                               @change="onChange">
                      <span v-for="(mem,i) in item.conditionList"
                            :key="i"
                        ...
                        <fox-button v-if="mem.compId=='001'"
                                    type="success"
                                    size="small"
                                    circle
                                    @click="symbolChange(item.ruleType)">{{mem.compName}}</fox-button>
                        <fox-button v-else-if="mem.compId=='002'"
                                    type="danger"
                                    size="small"
                                    circle
                                    @click="symbolChange(item.ruleType)">{{mem.compName}}</fox-button>
                        <span v-else-if="mem.compId=='003'||mem.compId==='004'"
                              size="small"
                              class="bracket">{{mem.name}}</span>
                        <fox-button v-else
                                    size="small"
                                    @dblclick.native="condition">{{mem.desc?mem.desc:mem.compName}}</fox-button>
                      </span>
                    </draggable>
                  </div>
                </fox-item>
              ...
              <fox-button-row class="right-button">
                <fox-button size="small"
                            @click.prevent="removeRule(item, index)">删除规则</fox-button>
                <fox-button size="small"
                            @click="addEvent(index)">添加事件</fox-button>
              </fox-button-row>
            </section>
            <event :event-operation="eventOperation"
                   :event-list="item.eventList"
                   :rule-type="item.ruleType"
                   @removeEvent="removeEvent($event,index)"></event>
          </div>
        </fox-form>
        <fox-group label-width="80px">
          <fox-button-row align="center">
            <fox-button type="success"
                        size="mini"
                        @click="doSave()">提交</fox-button>
            <fox-button type="primary"
                        size="mini"
                        @click="preAddRule">添加规则</fox-button>
            <fox-button type="danger"
                        size="mini"
                        @click="reset">重置</fox-button>
          </fox-button-row>
        </fox-group>
      </fox-main>
      <fox-aside class="right-aside"
                 style="width: 18vw">
        <draggable :list="symbolList"
                   v-bind="{group:{name:sign1, pull:'clone'}, filter:'.undraggable', sort:false}"
                   @start="start1"
                   @end="end1">
          <span v-for="(item, index) in symbolList"
                :key="index">
            <fox-button v-if="item.compId!=='001'&&item.compId!=='002'"
                        class="aside-button"
                        slot="reference"
                        round>{{item.compId === '003' ? (item.name + ' )') : item.compName}}</fox-button>
          </span>
        </draggable>
        <draggable :list="componentList"
                   ...
                   @start="start2"
                   @end="end2">
          <span v-for="(item, index) in componentList"
                :key="index">
            <fox-button class="aside-button"
                        slot="reference"
                        round>{{item.compName}}</fox-button>
          </span>
        </draggable>
      </fox-aside>
    </fox-container>

    <vueContextMenu id="menu"
                    :context-menu-data="menuData"
                    style="padding-left:0"
                    @remove="remove"
                    @symbol="symbol"
                    @condition="condition"></vueContextMenu>

    <fox-dialog :title="innerTitle"
      ...
      <fox-container>
        <fox-main class="left-p0">
          <fox-form ref="conditionForm"
                    :model="conditionForm">
            <fox-group label-width="80px"
                       column="1"
                       label-position="right">
              <fox-text-item v-model="conditionForm.compName"
                             @input="$forceUpdate()">条件名称</fox-text-item>
            </fox-group>
            <fox-divider content-position="left">参数</fox-divider>
            <fox-group label-width="80px"
                       column="1"
                       label-position="right">
              <fox-item v-for="(item, index) in conditionForm.paramForm"
                        :label="item.paramName"
                        :key="index">
                <fox-text-item v-if="!item.paramSource||item.paramSource ==='NULL'"
                               v-model="item.paramPath"
                               @blur="selectInput(index)"
                               @clear="item.paramComment=''"></fox-text-item>
                <fox-select-item v-else
                                 v-model="item.paramPath"
                                 :source="m_compareType"
                                 placeholder="请选择"></fox-select-item>
              </fox-item>
              <fox-button-row align="center">
                <fox-button type="success"
                            size="mini"
                            @click="sync">确定</fox-button>
                <fox-button type="danger"
                            size="mini"
                            @click="innerClear">取消</fox-button>
              </fox-button-row>
            </fox-group>
          </fox-form>
        ...
        <fox-aside>
          <fox-tabs v-model="activeName"
                    type="border-card">
            <fox-tab-pane label="公共报文"
                          name="public">
              <fox-table-item :content="m_publicParamData"
                              stripe
    ...
    <fox-dialog :title="innerTitle1"
                :visible.sync="innerVisible1"
                width="500px"
                append-to-body
                center>
      <fox-form ref="ruleForm"
                :model="ruleForm">
        <fox-group column="1"
                   label-width="80px">
          <fox-select-item v-model="ruleForm.ruleType"
                           :source="m_ruleType"
                           :rules="{
                            required: true, message: '规则类型不能为空', trigger: 'blur'
                        }"
                           label="规则类型"></fox-select-item>
          <fox-select-item :source="queryPublicRules"
          ...
          <fox-button-row align="center">
            <fox-button type="success"
                        size="medium"
                        @click="addRule">确定</fox-button>
            <fox-button type="danger"
                        size="medium"
                        @click="innerVisible1=false">取消</fox-button>
          </fox-button-row>
        </fox-group>
      </fox-form>
    </fox-dialog>
...
export default {
    ...
    methods: {
        ...
        add(ev) {
            ...
            if (
                ['001', '002'].indexOf(temp[index].compId) === -1
        && temp[index - 1]
        && ['001', '002'].indexOf(temp[index - 1].compId) === -1
        && temp[index - 1].compId !== '003'
            ) {
                temp.splice(index, 0, b)
            }
            if (
                temp[index - 1]
        && ['001', '002', '003'].indexOf(temp[index - 1].compId) !== -1
        && (!temp[index + 1].flag || temp[index + 1].compId === '003')
        && temp[index].compId !== '003'
            ) {
            ...
            if (ev.moved) {
                ...
                if (a < b) {
                    if (!temp[b - 1].flag) {
                        ...
                        ['001', '002'].indexOf(temp[b - 1].compId) !== -1
            && (!temp[b + 1].flag || temp[b + 1].compId === '003')
                    ) {
                    ...
                    if (temp[b - 1].compId === '003') {
                        ...
                        if (
                            temp[a - 1]
              && ['001', '002'].indexOf(temp[a - 1].compId) !== -1
                        ) {
                            temp.splice(a - 1, 1)
                ...
                if (a > b) {
                    if (['001', '002'].indexOf(temp[b + 1].compId) !== -1) {
                        if (
                            temp[a + 1]
              && ['003', '004'].indexOf(temp[a + 1].compId) === -1
                        ) {
                            temp.splice(a + 1, 1)
        ...
        symbol() {
            ...
            b.flag = true
            if (
                this.temp.ruleList[this.itemIndex].conditionList[this.index].compId
        === '001'
            ) {
                this.temp.ruleList[this.itemIndex].conditionList.splice(
                    this.index,
                    1,
                    b,
                )
        ...
        enter(index, item, itemIndex) {
            ...
            this.conditionForm.compName = item.conditionList[index].desc
                ? item.conditionList[index].desc
                : item.conditionList[index].compName
            ...
            if (['001', '002'].indexOf(item.conditionList[index].compId) !== -1) {
                ...
                ['003', '004'].indexOf(item.conditionList[index].compId) !== -1
            ) {
                this.menuData.menulists = [
                    { fnHandler: 'remove', icoName: 'el-icon-delete', btnName: '删除' },
                ]
                    ...
                    if (temp[j].count === temp[index].count && temp[j].compId === '003') {
                        if (index - j === 1) {
                            ...
                            if (temp[j] && ['001', '002'].indexOf(temp[j].compId) > -1) {
                                ...
                                temp[j - 1]
                && ['001', '002'].indexOf(temp[j - 1].compId) > -1
                            ) {
                                temp.splice(j - 1, 1)
                            }
                        } else {
        ...
        // 条件参数配置
        condition() {
            let condition = this.temp.ruleList[this.itemIndex].conditionList[
                this.index
            ]
